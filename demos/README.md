...
| [`03_prefix_cache_router.py`](03_prefix_cache_router.py) | Prefix-aware routing over a shared offload tier; TTFT win on long shared prefixes |
| [`04_kv_handoff_push_pull.py`](04_kv_handoff_push_pull.py) | KV-cache transfer modes between prefill and decode workers, and their TTFT timelines |
| [`05_rmsnorm_numerics.py`](05_rmsnorm_numerics.py) | How an FP32-vs-native-dtype final multiply causes RMSNorm drift to compound across layers |
...
2. **`02`** — shows the failure mode (ITL spikes, head-of-line blocking) and goodput as the honest metric.
3. **`04`** — fills in the missing piece: how KV moves between disaggregated workers (push vs pull).
4. **`03`** — extends the cache hierarchy to a shared cross-replica tier and shows the TTFT routing win.
5. **`05`** — a kernel-numerics aside; small in scope, large in compounding effect.
